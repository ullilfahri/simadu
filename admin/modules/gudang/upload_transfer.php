...
$new_name = $id_pengambilan.'-'.$distributor.'-12.' . $ext;
...
if(in_array($ext, $allowed_ext) && $size <= (1024*1024)) {
	$path = $_SERVER['DOCUMENT_ROOT']."/folder_upload/file_cetak/file_transfer_retur/" . $new_name; 
	$eks=move_uploaded_file($tmp_file, $path);
	$ada = $conn->query("SELECT * from tabel_arsip WHERE id_trx='$id_pengambilan' AND distributor='$distributor' AND id_jenis ='12' AND no_faktur='$no_faktur' ")->num_rows;	
	if ($ada == 0) {
	$ket='Simpan';
	$masuk=$conn->query(" INSERT INTO `tabel_arsip`(`id_toko`, `id_jenis`, `no_faktur`, `id_trx`, `distributor`, `file`) VALUES ('$id_toko', '12', '$no_faktur', '$id_pengambilan', '$distributor', '$new_name') ");
	} else {
	$ket='Update File';
	$masuk=$conn->query(" UPDATE `tabel_arsip` SET file = '$new_name' WHERE id_trx='$id_pengambilan' AND distributor='$distributor' AND id_jenis ='12' AND no_faktur='$no_faktur' ");
	}
	if ($eks) {
