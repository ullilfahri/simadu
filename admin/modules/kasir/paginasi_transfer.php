...
if ($uri=='hari_ini') {
	...
	$small='Hari ini';
	$trx=$conn->query("SELECT * FROM tabel_retur where id_toko LIKE '%".$_SESSION['toko']."%' AND no_faktur between $awal and $akhir  AND kode=1 ORDER BY id_trx ASC");
	...
	$trx=$conn->query("SELECT * FROM tabel_retur where id_toko LIKE '%".$_SESSION['toko']."%' AND id_trx between $awal and $akhir  AND kode=1 ORDER BY id_trx ASC");
                                ...
                                <table class="table table-striped table-bordered" id="data-table-example4">
									...
									while ($x=$trx->fetch_array()) :
									  if ($x['proses']==0) {
										...
										$ada_arsip=$conn->query("SELECT file FROM `tabel_arsip` WHERE `id_jenis`=3 AND `id_trx`='".$x['id_trx']."'")->fetch_array();
										if ($ada_arsip[0]==0) {
											...
											$target='_blank';
											$disabled='';
											$href='../pdf_viewer/dari_kasir.php?dir=penjualan&file='.$ada_arsip[0];
											}
									...
									?>
                                        <tr>
                                            ...
                                            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                                                ...
                                                <button type="button" class="btn btn-info waves-effect" data-toggle="modal" data-target=".upload" data-id="<?=$x['id_trx']?>" data-faktur="<?=$x['no_faktur']?>" data-distributor="<?=$x['distributor']?>" data-uri="<?=$modal_uri?>"  <?=$ijin?>><i class="fa fa-upload fa-lg"></i></button>
                                                </div>
                    ...
                    <div class="modal-dialog modal-lg">
                      <div class="modal-content">
                        ...
                        <div class="modal-header">
                          ...
                          <h4 class="modal-title" id="myModalLabel">Upload Dokumen Retur</h4>
                        </div>
   ...
   <script type="text/javascript">
    $(document).ready(function(){
        $('.upload').on('show.bs.modal', function (e) {
			...
			var no_faktur = $(e.relatedTarget).data('faktur');
            $.ajax({
                type : 'post',
                url : 'upload_transfer.php?modal_upload',
                data :  'id_trx='+id_trx+'&distributor='+distributor+'&uri='+uri+'&no_faktur='+no_faktur,
