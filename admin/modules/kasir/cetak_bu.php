...
if ($metode_bayar==0 && $uang_muka == 0) :
	if ($dalam_bank > 0) :
	$masuk=$conn->query("INSERT INTO `tabel_bayar`(`distributor`, `no_faktur`, `jumlah_transaksi`, jumlah_bayar , `tgl_transaksi`, `customer`, `kode_bayar`, id_toko) SELECT distributor, no_faktur, SUM(sub_total), '0', '".$tgl_transaksi."', '".$customer."', '2', '$id_toko' from tabel_trx where no_faktur='$no_faktur' AND distributor='".$ulang[0]."' GROUP BY distributor");
	endif;
endif;
...
if ($metode_bayar==0 && $uang_muka > 0 && $uang_muka != $total_awal && $jumlah_distributor < 3) :
	...
	/*
	$cari=$conn->query("SELECT COUNT(*), `sub_total`, distributor, SUM(sub_total) FROM tabel_trx WHERE no_faktur='$no_faktur' AND sub_total IN (SELECT `sub_total` FROM tabel_trx GROUP BY `distributor` HAVING SUM(`sub_total`) >= '$jatah_awal') ")->fetch_array();
	*/
