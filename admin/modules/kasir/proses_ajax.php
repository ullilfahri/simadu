...
if (isset($_GET['input'])) {
		...
		echo " <i class='fa fa-ban fa-lg' style='color:red;'></i> Tidak Ada Barang ".$input;
			...
			if ($act ==0)	:
            ?>
            <i class='fa fa-check fa-lg' style='color:green;'>  </i><a href="javascript:autoInsert('<?=$data[0]?>','<?=$data[4]?>','<?=htmlspecialchars($data[3])?>','<?=angka($stok['harga_barang'])?>','<?=$stok['sisa_stok']?>','<?=$data[1]?>','<?=$ada_jual?>','<?=$lokasi[0]?>','<?=$stok['satuan']?>','<?=$data[5]?>','<?=$data[2]?>','<?=$level[1]?>','<?=$level[2]?>','<?=$level[3]?>');"><?=$data[4].'-'.$data[3].' '.$lokasi[0] ?><br> <!--- hasil
